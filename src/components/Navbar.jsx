    ...
    >
      <div className="group relative flex items-center justify-between w-full max-w-7xl px-4 py-2 bg-white/40 dark:bg-neutral-900/40 backdrop-blur-xl border border-white/40 dark:border-white/5 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] transition-all duration-300 hover:shadow-[0_20px_50px_rgba(0,0,0,0.2)]">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={toggleTheme}
            className="p-2.5 rounded-xl border cursor-pointer border-slate-900/30 dark:border-white/5 bg-white/40 dark:bg-white/5 hover:bg-white/20 hover:shadow-xl transition-all text-slate-600 dark:text-slate-400"
          >
            {theme === "dark" ? (
              <RiSunLine className="text-[20px] text-cyan-400" />
            ) : (
              <RiMoonClearLine className="text-[20px] text-slate-900/80" />
            )}
          </button>
